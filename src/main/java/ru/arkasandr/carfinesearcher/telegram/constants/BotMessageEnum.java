...
public enum BotMessageEnum {
    ...
    EXCEPTION_MULTIPLY_SENDING("Повторное отправление было отклонено!"),

    HELP_MESSAGE("Бот предназначен для поиска штрафов в ГИБДД посредством ввода регистрационного номера "
            + "и свидетельства о регистрации транспортного средства. \n"
            + "Данные вводятся последовательно - сначало регистрационный номер, затем свидетельство о регистрации. \n"
            ...
            + "Затем создайте новый запрос с корректными данными. \n"
            + "При наличии незаконченного запроса отсутствует возможность создать и отправить следующий. \n"
            + "Во время проверки требуется ввод проверочного кода. Допускается три попытки для его распознавания. \n"
